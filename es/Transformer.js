...
import { camelCase, padding, startsWith, transformAbsoluteToRelative } from './utils';
...
assign(MLTransformer.prototype, {
  ...
  generateCodeForTag: function generateCodeForTag(node, level_) {
    ...
    if (tag === 'import') {
      var importPath = transformAbsoluteToRelative(projectRoot, renderPath, attrs.src, allowImportModule);
      importTplDeps[importPath] = importTplDeps[importPath] || this.importIncludeIndex++;
      return;
    }

    if (tag === 'include') {
      var includePath = transformAbsoluteToRelative(projectRoot, renderPath, attrs.src, allowImportModule);
    ...
    if (this.FOR_ATTR_NAME in attrs) {
      ...
      this.scope.push((_scope$push = {}, _defineProperty(_scope$push, indexName, 1), _defineProperty(_scope$push, itemName, 1), _scope$push));
